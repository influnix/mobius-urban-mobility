...
서울 지하철 240개 역의 일별 총 승하차 수요를 대상으로 단순 기준모델과 달력 인식 기준모델을 구축했다.
...
- 평가 단위: 역 x 날짜
- 평가 관측치: 22,080개
- 평가 방식: Rolling one-day-ahead

날짜 (t)를 예측할 때 (t-1)일까지 관측된 실제 수요만 사용한다.
...
- 전일 수요: `lag_1`
- 전주 동일 요일 수요: `lag_7`
- 최근 4주 동일 요일 평균: `lag_7`, `lag_14`, `lag_21`, `lag_28` 평균

`lag_7`은 전체 WAPE 11.209%로 가장 낮았지만, 공휴일 WAPE가 95.310%에 달했다.
예측일과 7일 전 날짜의 유형이 다를 때 평일 수요를 공휴일에 복사하거나 공휴일 수요를 다음 주 평일에 복사하는 구조적 문제가 발견됐다.

### 달력 인식 기준모델

다음 우선순위 규칙을 적용했다.

1. 예측일과 7일 전의 날짜 유형이 같으면 `lag_7`
2. 날짜 유형이 다르고 동일 유형의 과거 표본이 3개 이상이면 역별 과거 동일 날짜 유형 중앙값
3. 과거 표본이 부족하면 `lag_1`

현재 날짜의 실제 수요가 중앙값 계산에 포함되지 않도록 `shift(1)` 이후 누적 중앙값을 계산했다.

### 주요 결과

| 지표    | 기존 `lag_7` | 달력 인식 기준모델 |
| ----- | ---------: | ---------: |
| MAE   |  4,227.638 |  2,104.852 |
| RMSE  | 10,726.562 |  4,868.435 |
| WAPE  |    11.209% |     5.581% |
| sMAPE |    13.937% |     6.824% |
| Bias  |     0.798% |     0.360% |
...
| 날짜 유형 | 기존 `lag_7` | 달력 인식 기준모델 |
| ----- | ---------: | ---------: |
| 일반 평일 |     7.143% |     3.687% |
| 일반 주말 |     8.628% |     8.574% |
| 공휴일   |    95.310% |    24.299% |

92일 중 두 모델의 예측이 실제로 달라진 날짜는 13일이었다.
...
2025년 12월의 실제 수요를 100배로 변경한 뒤 다시 계산해도 2025년 10월 예측의 최대 차이는 0.0으로 나타났다. 현재 파이프라인에서는 미래 실제값이 과거 예측에 역방향으로 전달되는 형태의 누수가 발견되지 않았다.

### 한계

* 모든 공휴일을 하나의 `Public holiday`로 처리한다.
* 명절, 연휴 내 위치, 공휴일 다음 날을 구분하지 못한다.
* 2025년 한 해와 10~12월 단일 홀드아웃에 기반한다.
* Rolling one-day-ahead 조건의 결과이며 고정 시점 92일 장기 예측 결과는 아니다.
* 날씨, 행사, 운행 변화 등의 외부 변수를 사용하지 않았다.

### 실행

```bash
python src/09_demand_forecast_baselines.py
python src/10_calendar_aware_baseline.py
```
